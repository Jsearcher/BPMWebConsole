...
namespace BPMWebConsole.Models.DAO
{
    ...
    public class BPMLast : DBRecord
    {
        ...
        public BPMLast(IDbConnection pConn) : base(pConn)
        {
            DBOwner = "dbo";
            TableName = "BPM_LAST";
        ...
        /// <summary>
        /// 依據[BSM_FLIGHT]、[BSM_DATE]篩選[dbo.BPM_LAST]資料表，[BSM_FLIGHT]包含航空公司代碼
        /// </summary>
        ...
        public int SelectByDate(string pDate)
        {
            pDate = string.IsNullOrEmpty(pDate) ?
                DateTime.Now.ToString("yyyyMMdd") :
                DateTime.ParseExact(pDate, "yyyyMMdd", CultureInfo.InvariantCulture).ToString("yyyyMMdd");
            return SelectByCondition(string.Format(" WHERE {0} = '{1}'" +
                                                    " ORDER BY {2}, {3}",
                                                    FieldName[0], pDate, FieldName[9], FieldName[1]));
        }

        /// <summary>
        /// 依據[BAG_TAG], [BSM_DATE]篩選[dbo.BPM_LAST]資料表
        /// </summary>
